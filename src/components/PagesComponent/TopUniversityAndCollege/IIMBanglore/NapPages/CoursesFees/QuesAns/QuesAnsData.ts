const QuesAnsData = [
  {
    user: "Anshul Mittal",
    date: "June 1, 2019",
    questions: [
      {
        question:
          "How tough is IIM B test for getting into fellowship program at IIM...?",

        answer:
          "Here are some useful tips which will help prepare for IIM Banagalore interview.The IIM B test into fellowship program which is known as RAT difficult It can be challenging ., The IIM B test into fellowship program which is known as RAT difficult It can be challenging .IIM Banagalore has a beautiful campus Life at IIMB campus is enriching.Here are some useful tips which will help prepare for IIM Banagalore interview.",
      },
    ],
  },

  {
    user: "Akash Mittal",
    date: "May 5, 2020",
    questions: [
      {
        question:
          "How should I go about with the prepration for IIM Banagalore interiview?",

        answer:
          "Here are some useful tips which will help prepare for IIM Banagalore interview.The IIM B test into fellowship program which is known as RAT difficult It can be challenging ., The IIM B test into fellowship program which is known as RAT difficult It can be challenging .IIM Banagalore has a beautiful campus Life at IIMB campus is enriching.Here are some useful tips which will help prepare for IIM Banagalore interview.Here are some useful tips which will help prepare for IIM Banagalore interview.The IIM B test into fellowship program which is known as RAT difficult It can be challenging .",
      },
    ],
  },
  {
    user: "Shiva Mittal",
    date: "June 1, 2020",
    questions: [
      {
        question: "How is campus life of students at IIM Banagalore?",

        answer:
          " IIM Banagalore has a beautiful campus Life at IIMB campus is enriching. Here are some useful tips which will help prepare for IIM Banagalore interview.The IIM B test into fellowship program which is known as RAT difficult It can be challenging ., The IIM B test into fellowship program which is known as RAT difficult It can be challenging .IIM Banagalore has a beautiful campus Life at IIMB campus is enriching.Here are some useful tips which will help prepare for IIM Banagalore interview.Here are some useful tips which will help prepare for IIM Banagalore interview.",
      },
    ],
  },
];
export default QuesAnsData;
